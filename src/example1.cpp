...
//----------------------------------------------------------------------------
vec4 centralMass(particle part) {
    vec4 t_com;
    point4 poss_mass;
    float temp_mass;

    for( int i = 0; i < num_particles; i++) {
        poss_mass += particles[i].position * particles[i].mass;
        temp_mass += particles[i].mass;
    }
    poss_mass /= temp_mass;
    t_com = poss_mass - part.position;
    return t_com;
}
...
idle( void )
{
    ...
    for ( i = 0; i < num_particles; i++ ) {
        for ( j = 0; j < 3; j++ ) {
            ...
            particles[i].velocity[j] +=
                dt * forces( i, j ) / particles[i].mass + centralMass(particles[i])[j] * 0.045;
        }
        collision( i );
    }
    if ( repulsion )
